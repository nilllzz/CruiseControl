...
namespace CruiseControl.GameWorld.Objects.Vehicles.Wheels
{
    class ReichswagenWheel : Wheel
    {
        ...
        public ReichswagenWheel(IObjectContainer parent, Vehicle owner, Vector3 position, WheelType type, float size)
        ...
        {
            Texture = GetComponent<Resources>().LoadTexture("Textures/Vehicles/Wheels/rw_wheel.png");
        ...
        protected override void CreateGeometry()
        {
            lock (_vertexCache)
            {
                if (!_vertexCache.ContainsKey(_size))
                {
                    ...
                    _vertexCache.Add(_size, CylinderComposer.Create(_size, 0.2f, 20, sideTexture, topTexture));
                }
